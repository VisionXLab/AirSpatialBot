...
gt_json = "airspatial_agent_test_task1.jsonl"
action_json = "action_json/output_task1_geoground+internvl_40b.json"
answer_json = "execute_json/answer_task1_geoground+internvl_40b.json"
eval_json = "evaluate_json/evaluate_task1_geoground+internvl_40b.json"

vlm_name = "geoground"
llm_name = "internvl2-40b-2"
...
async def process_data(data: pd.DataFrame, llm_name: str):
    output_data = {}
...
if __name__ == "__main__":
    ...
    loop = asyncio.get_event_loop()
    results = loop.run_until_complete(process_data(data, llm_name))
    with open(action_json, "w") as f:
        json.dump(results, f, indent=4)

    # execute planning
    # with open("action_json/output_task1_internvl2_40b.json", "r", encoding="utf-8") as f:
    #     results = json.load(f)
    ...
    # evaluate
    # with open("execute_json/output_task1_internvl2_40b.json", "r", encoding="utf-8") as f:
    ...
    for index, row in answer_dict.items():
        question, gt = row["question"], str(row["gt"])
        ...
        if isinstance(tmp_answer, dict):
            for k, v in tmp_answer.items():
                if isinstance(v, list):
                    answer = answer.join(v)
                else:
                    answer = answer + str(v)
        else:
            answer = tmp_answer
        ...
        tmp = {
            ...
            "gt": gt,
            "answer": answer,
            "evaluate": True if str(gt) in str(answer) else False,
            "number": q_type,
        }
